...
The workspace substrate should be different from Codex, Claude Code, opencode, and OpenHands in five ways:
...
3. The transcript is a control channel, not hidden memory.
   Durable memory lives in workspace files. Turn-to-turn prompt replay carries compact action receipts, bounded recent observations, snapshots, and an optional short `<note>` intent anchor; it does not preserve arbitrary prose or durable edit bodies as a second filesystem.

4. The prompt/tool surface is intentionally small.
...
5. Benchmarks drive the architecture.
